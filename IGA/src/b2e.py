...
""")
  ...
  for i in range(len(ien_offset)-1):
    ...
    if options.make_data:
      edge_1 = result_xyz_[:divide]
      edge_2 = result_xyz_[-divide:]
      edge_3 = [result_xyz_[m*divide] for m in range(divide)]
      edge_4 = [result_xyz_[m*divide + divide-1] for m in range(divide)]
      numpy.savetxt(os.path.join("EDGE_DATA","edge_{0:1d}_1.txt".format(i)),edge_1)
      numpy.savetxt(os.path.join("EDGE_DATA","edge_{0:1d}_2.txt".format(i)),edge_2)
      numpy.savetxt(os.path.join("EDGE_DATA","edge_{0:1d}_3.txt".format(i)),edge_3)
      numpy.savetxt(os.path.join("EDGE_DATA","edge_{0:1d}_4.txt".format(i)),edge_4)
    for k in range(ele):
      for j in range(ele):
